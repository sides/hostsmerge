...
import getopt, sys, os, re, socket, time, urllib.request, shutil
...
def get_list(url):
	...
	return parse_list(urllib.request.urlopen(urllib.request.Request(url, headers={"User-Agent": "Python/urllib2"})).read().decode('utf-8'))
...
def get_hosts(url):
	...
	return parse_hosts(urllib.request.urlopen(urllib.request.Request(url, headers={"User-Agent": "Python/urllib2"})).read().decode('utf-8'))
...
def merge_rules(opts):
	...
	for source in opts["sources"]:
		...
		for ip, hostnames in new_hosts.items():
			if ip in hosts:
				for hostname in hostnames:
